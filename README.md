...
기능: 서버에 저장된 게시글 목록(제목, 작성자, 인덱스)을 조회하고 사용자에게 표시.
 ...
 서버로부터 JSON 데이터 배열을 수신하여 파싱(org.json.simple)하고, ArrayList<TextView>를 활용하여 게시판 목록을 동적으로 생성 및 업데이트하는 UI 로직 구현.
...
3) 게시글 CRUD (생성, 조회, 수정, 삭제) 기능 구현
생성 (Create): 게시글 제목 및 내용을 입력받아 서버로 전송(_2_1write_page.java).

조회 (Read): 목록에서 선택한 게시글의 상세 내용(제목, 내용, 작성자)을 서버에서 조회하고 화면에 출력 
